...
public class ProductController {
    ...
    @GetMapping("/{id}")
    @ApiOperation("Search for a product with am specific ID")
    @ApiResponses({
            @ApiResponse(code = 200, message = "OK"),
            @ApiResponse(code = 404, message = "Product not found")
    })
    public ResponseEntity<Product> getProduct (@ApiParam(value = "The id of the product to search", required = true, example = "7") @PathVariable("id") int productId) {
